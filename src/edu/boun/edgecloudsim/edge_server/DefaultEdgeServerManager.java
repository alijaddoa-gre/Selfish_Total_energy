...
public class DefaultEdgeServerManager extends EdgeServerManager
{
...

	{
		...
		int k= SimSettings.getInstance().getMaxNumOfMobileDev();
 ...
 * (non-Javadoc)
 * @sALI
 * we have assumed that each app (ioT) uses one data centre in the edge.
 *  as well as, it starts from the data centre 4 as the first four data centres are represent the for IoT devices. 
 */
	public void createVmList(int brockerId)
	{
		...
		for (int i = 0; i < datacenterList.getLength(); i++) 
		//for (int i = 4; i < 8; i++)  // four data centres for four application
		{
			Node datacenterNode = datacenterList.item(i);
			Element datacenterElement = (Element) datacenterNode;
			...
			for (int j = 0; j < hostNodeList.getLength(); j++)
			{
				...
				//System.out.println("aAAAaaaaaaaaaa"+vmNodeList.getLength());
				for (int k = 0; k < vmNodeList.getLength(); k++)
				{
					...
					bandwidth = 0;//SimSettings.getInstance().getWlanBandwidth(); // / (hostNodeList.getLength()+vmNodeList.getLength());
			...
			//System.out.println();
			//System.out.println("Edge VM"+ i+"  "+ mips);
		}
	}
	...
	private Datacenter createDatacenter(int index, Element datacenterElement) throws Exception
	{
		...
		//System.out.println();
		//System.out.println("edge   DataCentre "+ datacenter.getId());
	...
	private List<EdgeHost> createHosts(Element datacenterElement){
		...
		for (int j = 0; j < hostNodeList.getLength(); j++) {
			...
			hostIdCounter++;
			//System.out.println();
			//System.out.println("Edge   Host "+ host.getId());
		}
